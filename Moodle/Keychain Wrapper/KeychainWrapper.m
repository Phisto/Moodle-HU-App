...
- (void)resetKeychainItem
{
    ...
    else if (_keychainData)
    {
        ...
        OSStatus errorcode __unused = SecItemDelete((__bridge CFDictionaryRef)tmpDictionary);
        NSAssert(errorcode == noErr, @"Problem deleting current keychain item." );
    }
    
    // Default generic data for Keychain Item:
    [_keychainData setObject:@"MOODLE Login" forKey:(__bridge id)kSecAttrLabel];
    [_keychainData setObject:@"The login credentials for a MOODLE account." forKey:(__bridge id)kSecAttrDescription];
    [_keychainData setObject:@"Account" forKey:(__bridge id)kSecAttrAccount];
    [_keychainData setObject:@"moodle.simonsserver.de" forKey:(__bridge id)kSecAttrService];
    [_keychainData setObject:@"No comment." forKey:(__bridge id)kSecAttrComment];
...
- (void)writeToKeychain {
                            ...
                            (CFTypeRef *)&attributes) == noErr)
    {
        ...
        OSStatus errorcode __unused = SecItemUpdate(
    ...
    else
    {
        ...
        OSStatus errorcode __unused = SecItemAdd(
